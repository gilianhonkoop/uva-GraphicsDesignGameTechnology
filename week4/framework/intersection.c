 ...
 *
 * Student name ... Gilian Honkoop & Jan Deen
 * Student email ...
 * Collegekaart ... 13710729 & 12873551
 * Date ........... 7-10-2022
 * Comments ....... Speeding up raytracing using BVH.
 *
 *
...
void
intersected_recursion(intersection_point* ip, vec3 ray_origin, vec3 ray_direction,
    float t0, float t1, bvh_node *current)
{
    // If the node is a leaf, check for triangle intersections.
    if (current->is_leaf){

        int num = leaf_node_num_triangles(current);
        triangle* tri = leaf_node_triangles(current);

        intersection_point ip2;

        // Set ip to the closest triangle-intersection point.
        for(int i = 0; i < num; i++) {
            if(ray_intersects_triangle(&ip2, tri[i], ray_origin, ray_direction)) {
        ...
        bvh_node *left = inner_node_left_child(current);
        bvh_node *right = inner_node_right_child(current);
        ...
        // Check if the ray intersects the left- and/or right-node bbox and set tmin/tmax.
        int leftcheck = bbox_intersect(&tmin1, &tmax1, left->bbox, ray_origin, ray_direction, t0, t1);
        int rightcheck = bbox_intersect(&tmin2, &tmax2, right->bbox, ray_origin, ray_direction, t0, t1);

        // Traverse the closest box first.
        if (tmin1 <= tmin2) {

            // Enter the next node if the bbox start is closer than a previously found triangle.
            if( leftcheck && tmin1 < ip->t) {

                intersected_recursion(ip, ray_origin, ray_direction, tmin1, tmax1, left);
            }

            // Enter the next node if the bbox start is closer than a previously found triangle.
            if( rightcheck && tmin2 < ip->t) {

                intersected_recursion(ip, ray_origin, ray_direction, tmin2, tmax2, right);
            }
        }
    ...
    vec3 ray_origin, vec3 ray_direction)
{
    ...
    // Get the t0-t1 values for the root bbox.
    bbox_intersect(&tmin, &tmax, bvh_root->bbox, ray_origin, ray_direction, t0, t1);
    t0 = tmin;
    t1 = tmax;

    // Traverse the bboxes and look for intersection point.
    intersected_recursion(ip, ray_origin, ray_direction, t0, t1, bvh_root);

    // If an intersection point is found, return 1.
    if (ip->t < INFINITY) {
        return 1;
    }
